...
class DnDStatManager(tk.Tk):
    ...
    def open_page(self, node: Node, view_type: str, stat_path: Path = None, data: dict = None, is_reference_click: bool = False):
        ...
        if is_reference_click:
            ...
            if node.level == 0:
                ...
                parent_type = "root_folder" if (parent_path.resolve() == self.root_dir.resolve() or parent_path.resolve() in [self.map_dir.resolve(), self.events_dir.resolve(), self.objects_dir.resolve(), self.spells_dir.resolve(), self.monsters_dir.resolve(), self.npcs_dir.resolve(), self.combats_dir.resolve()]) else "event" if (parent_path.resolve() == self.events_dir.resolve() or self.events_dir in parent_path.parents) else "location"
    ...
    def display_monster(self, node: Node, edit_mode=False, back_cb=None):
        ...
        try:
            ...
            if edit_mode: 
                # MODIFIED: Include Objects search selection dialog trigger inside character editors signatures
                self.stat_viewer.render_edit_mode(data, node.path, loc_name, self.save_monster_edits, lambda: self.display_monster(node, edit_mode=False, back_cb=back_cb),
                                                   add_existing_object_cb=lambda row: EntitySelectionDialog(self, self.objects_dir, "Objects", row))
            else:
    ...
    def save_monster_edits(self, old_dir: Path, new_data: dict):
        ...
        # ADDED: Symmetrical relationship sync for edited characters
        self.sync_reciprocal_relations(final_json)
        self.refresh_tree_silent()
    ...
    def refresh_tree_silent(self):
        ...
        objects_node = Node(name="Objects", path=self.objects_dir, is_entity=False, level=0, icon_path=Path("./utils/objects.png") if Path("./utils/objects.png").exists() else None, is_open=str(self.objects_dir) in open_paths)
        objects_node.children = self.build_tree_model(self.objects_dir, level=1, open_paths=open_paths)
        
        self.nodes = [map_node, events_node, objects_node, spell_node, monsters_node, npcs_node, combats_node]
        self.render_tree()
    ...
    def build_tree_model(self, path: Path, level: int, open_paths: set):
        ...
        for item in files:
            ...
            node = Node(name=item.stem, path=item, is_entity=True, level=level, icon_path=icon if icon.exists() else None, stat_path=item)
            nodes.append(node)
        ...
        # ADDED: Enforce action trigger creation inside Objects root
        elif path == self.objects_dir:
            nodes.append(Node(name="Add Object", path=path, is_entity=False, level=level, action_type="new_object", icon_path=Path("./utils/new.png")))
    ...
    def render_tree(self):
        ...
        def draw_node(node):
            ...
            del_btn = None
            # MODIFIED: Guard "Objects" root folder text from receiving delete buttons
            if not is_action_btn and node.name not in ["Map", "Events", "Objects", "Spells", "Monsters", "NPCs", "Combats"]:
                ...
                def on_del_click(e, n=node):
                    ...
                    if messagebox.askyesno("Confirm Delete", msg):
                        try:
                            ...
                            self.sync_reciprocal_relations(None)
                            self.refresh_tree_silent()
                            self.clear_viewer_and_tree()
            ...
            def on_click(e):
                if node.action_type == "new_location": self.create_new_location(node.path)
                    ...
                    if node.path.parent == self.spells_dir:
                        ...
                        self.open_page(node, view_type="object", stat_path=node.stat_path, is_reference_click=False)
                    else:
                    ...
                    v_type = "root_folder" if node.path in [self.map_dir, self.events_dir, self.objects_dir, self.spells_dir, self.monsters_dir, self.npcs_dir, self.combats_dir] else "event" if "Events" in str(node.path) else "location"
                    self.open_page(node, view_type=v_type, is_reference_click=False)
                    self.render_tree()

            def on_icon_click(e):
                is_core_folder = (node.name in ["Monsters", "NPCs", "Combats", "Map", "Spells", "Events", "Objects"])
    ...
    def on_location_link_clicked(self, name, category):
        """Unified reference link router supporting lookups from both display panels and back-navigation context checks."""
        if self.stat_viewer.edit_container.winfo_ismapped():
            if self.current_state.view_type == "location": 
                self.current_state.view_type = "location_edit"
            elif self.current_state.view_type == "event": 
                self.current_state.view_type = "event_edit"
        ...
        # ADDED: Target branch enabling link routing directly to Object sheets
        elif category == "Objects":
            target_path = self.objects_dir / f"{name}.json"
            if not target_path.exists():
                found = list(self.objects_dir.glob(f"*{name}*.json"))
                if found: target_path = found[0]
            if target_path.exists():
                self.open_page(Node(name=target_path.stem, path=target_path, is_entity=True, level=1, stat_path=target_path), view_type="object", stat_path=target_path, is_reference_click=True)
            else:
                messagebox.showinfo("Not Found", f"Object reference data for '{name}' was not found.")
    ...
    def open_location_edit(self, node, stat_path, data):
        ...
        # FIXED: Aligned all 7 required entity selection callbacks in correct argument order
        self.stat_viewer.render_location_edit_mode(
            ...
            lambda row: EntitySelectionDialog(self, self.objects_dir, "Objects", row),
    ...
    def sync_reciprocal_relations(self, saved_path=None):
        """Processes cross-pollination linking across map regions, events log data, character files, and items profiles."""
        # 1. Symmetrical synchronization between map positions and event log manifests
        loc_data_map, evt_data_map = {}, {}
        for p in self.map_dir.rglob("*.json"):
            try:
                with open(p, "r", encoding="utf-8") as f: d = json.load(f); d.setdefault("events", []); loc_data_map[p] = d
            except: pass
        for p in self.events_dir.rglob("*.json"):
            try:
                with open(p, "r", encoding="utf-8") as f: d = json.load(f); d.setdefault("locations", []); evt_data_map[p] = d
            except: pass
        ...
        for lp, ld in loc_data_map.items():
            ...
            for e_name in ld.get("events", []):
                for ep, ed in evt_data_map.items():
                    if ed.get("name") == e_name or ep.parent.name == e_name:
                        if l_name not in ed["locations"]: ed["locations"].append(l_name)

        for ep, ed in evt_data_map.items():
            e_name = ed.get("name", ep.parent.name)
            for l_name in ed.get("locations", []):
                for lp, ld in loc_data_map.items():
                    if ld.get("name") == l_name or lp.parent.name == l_name:
                        if e_name not in ld["events"]: ld["events"].append(ep.parent.name)

        # Prune locations/events that no longer exist on disk
        existing_loc_names = {ld.get("name", lp.parent.name) for lp, ld in loc_data_map.items()}
        existing_evt_names = {ed.get("name", ep.parent.name) for ep, ed in evt_data_map.items()}
        for lp, ld in loc_data_map.items():
            ld["events"] = [e for e in ld["events"] if e in existing_evt_names]
        for ep, ed in evt_data_map.items():
            ed["locations"] = [l for l in ed["locations"] if l in existing_loc_names]
        ...
        # 2. Symmetrical synchronization between NPC/Monster configurations and Object possession templates
        creature_data_map, obj_data_map = {}, {}
        for p in list(self.monsters_dir.rglob("*.json")) + list(self.npcs_dir.rglob("*.json")):
            try:
                with open(p, "r", encoding="utf-8") as f: d = json.load(f); d.setdefault("objects", []); creature_data_map[p] = d
            except: pass
        for p in self.objects_dir.glob("*.json"):
            try:
                with open(p, "r", encoding="utf-8") as f: d = json.load(f); d.setdefault("owners", []); obj_data_map[p] = d
            except: pass

        existing_creature_names = {cd.get("name", cp.parent.name) for cp, cd in creature_data_map.items()}
        existing_obj_names = {od.get("name", op.stem) for op, od in obj_data_map.items()}

        if saved_path and saved_path.parent == self.objects_dir:
            # An object file was just saved. Its 'owners' list is the source of truth.
            saved_obj_data = obj_data_map.get(Path(saved_path))
            if saved_obj_data:
                o_name = saved_obj_data.get("name", Path(saved_path).stem)
                o_owners = saved_obj_data.get("owners", [])
                for cp, cd in creature_data_map.items():
                    c_name = cd.get("name", cp.parent.name)
                    if c_name in o_owners:
                        if o_name not in cd["objects"]: cd["objects"].append(o_name)
                    else:
                        if o_name in cd["objects"]: cd["objects"].remove(o_name)

        elif saved_path and (self.monsters_dir in Path(saved_path).parents or self.npcs_dir in Path(saved_path).parents):
            # A creature file was just saved. Its 'objects' list is the source of truth.
            saved_creature_data = creature_data_map.get(Path(saved_path))
            if saved_creature_data:
                c_name = saved_creature_data.get("name", Path(saved_path).parent.name)
                c_objects = saved_creature_data.get("objects", [])
                for op, od in obj_data_map.items():
                    o_name = od.get("name", op.stem)
                    if o_name in c_objects:
                        if c_name not in od["owners"]: od["owners"].append(c_name)
                    else:
                        if c_name in od["owners"]: od["owners"].remove(c_name)
        else:
            # Global fallback / deletion sync pass
            for cp, cd in creature_data_map.items():
                c_name = cd.get("name", cp.parent.name)
                for o_name in cd.get("objects", []):
                    for op, od in obj_data_map.items():
                        if od.get("name") == o_name or op.stem == o_name:
                            if c_name not in od["owners"]: od["owners"].append(c_name)

            for op, od in obj_data_map.items():
                o_name = od.get("name", op.stem)
                for c_name in od.get("owners", []):
                    for cp, cd in creature_data_map.items():
                        if cd.get("name") == c_name or cp.parent.name == c_name:
                            if o_name not in cd["objects"]: cd["objects"].append(o_name)

        # Enforce structural integrity against deleted entries
        for cp, cd in creature_data_map.items():
            cd["objects"] = [o for o in cd["objects"] if o in existing_obj_names]
        for op, od in obj_data_map.items():
            od["owners"] = [o for o in od["owners"] if o in existing_creature_names]

        for p, d in creature_data_map.items():
            try:
                with open(p, "w", encoding="utf-8") as f: json.dump(d, f, indent=4)
            except: pass
        for p, d in obj_data_map.items():
            try:
                with open(p, "w", encoding="utf-8") as f: json.dump(d, f, indent=4)
            except: pass
    ...
    def create_new_object(self):
        ...
        target_json = self.objects_dir / f"{sn}.json"
        if target_json.exists():
            messagebox.showerror("Error", "An entry with that name already exists here.")
            return
            
        default_obj = {"name": fn, "description": "", "effect": "", "owners": [], "trait": [], "action": [], "spellcasting": []}
        with open(target_json, "w", encoding="utf-8") as f: 
            json.dump(default_obj, f, indent=4)
            
        self.refresh_tree_silent()
        v_node = Node(name=fn, path=target_json, is_entity=True, level=1, stat_path=target_json)
        self.open_page(v_node, view_type="object", stat_path=target_json, is_reference_click=False)

    def open_object_edit(self, node, stat_path, data):
        def on_save(obj_file, updated_data):
            with open(stat_path, "w", encoding="utf-8") as f: 
                json.dump(updated_data, f, indent=4)
            old_name = stat_path.stem
            new_name = updated_data.get("name", old_name)
            new_safe = "".join([c for c in new_name if c.isalpha() or c.isdigit() or c in (' ', '-', '_')]).rstrip() or "Unnamed"
            
            final_json = stat_path
            if new_safe != old_name:
                target_json = stat_path.parent / f"{new_safe}.json"
                if target_json.exists():
                    messagebox.showerror("Error", f"An item configuration named '{new_safe}' already exists.")
                    return
                stat_path.rename(target_json)
                final_json = target_json
                
            self.sync_reciprocal_relations(final_json)
            self.refresh_tree_silent()
            
            # FIXED: Pass stat_path=final_json as an explicit parameter to open_page
            self.open_page(
                Node(name=new_name, path=final_json, is_entity=True, level=1, stat_path=final_json), 
                view_type="object", 
                stat_path=final_json, 
                is_reference_click=False
            )

        self.stat_viewer.render_object_edit_mode(
            data, stat_path, on_save, lambda: self._show_current_state_view(),
            lambda row: EntitySelectionDialog(self, self.npcs_dir, "NPCs", row)
        )
